...
@app.route('/office/<int:office_id>/increment', methods=['POST'])
def increment(office_id):
    if 1 <= office_id <= 3:
        command_queue.put({'type': 'increment', 'office_id': office_id})
        return jsonify({'success': True})
    return jsonify({'success': False}), 400

@app.route('/office/<int:office_id>/decrement', methods=['POST'])
def decrement(office_id):
    if 1 <= office_id <= 3:
        command_queue.put({'type': 'decrement', 'office_id': office_id})
        return jsonify({'success': True})
    return jsonify({'success': False}), 400

@app.route('/office/<int:office_id>/reset', methods=['POST'])
def reset_val(office_id):
    if 1 <= office_id <= 3:
