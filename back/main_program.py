...
@app.route('/')
def index():
    # Główny panel wszystkich gabinetów – serwujemy plik index.html
    return app.send_static_file('index.html')

@app.route('/status', methods=['GET'])
def status():
    # Zwrot aktualnego stanu: numbers, queues
    return jsonify(gui_instance.get_status())

@app.route('/office/<int:office_id>', methods=['GET'])
def office_view(office_id):
    # Widok konkretnego gabinetu – używa szablonu office.html
...
# ----------------------------------------------------------------
# JEDEN SPÓJNY ENDPOINT do wszelkich akcji: increment, decrement,
# reset, add_number, remove_number. Wywołujemy POST na:
#   /office/<office_id>/action/<action>
# gdzie <action> to jedna z ['increment','decrement','reset','add_number','remove_number']
# ----------------------------------------------------------------
...
def office_action(office_id, action):
    ...
    # Przy remove_number przyda się JSON z numerem do usunięcia
