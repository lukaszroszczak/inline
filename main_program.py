...
# ---------------------------
# Jeden endpoint: /office/<id>/action/<akcja>
# Obsługa increment, decrement, reset, add_number, remove_number, edit_number
# ---------------------------
@app.route('/office/<int:office_id>/action/<string:action>', methods=['POST'])
def office_action(office_id, action):
    if not (1 <= office_id <= 3):
        return jsonify({'success': False, 'error': 'Invalid office ID'}), 400

    data = request.get_json(silent=True) or {}

    if action == "increment":
        command_queue.put({'type': 'increment', 'office_id': office_id})

    elif action == "decrement":
        command_queue.put({'type': 'decrement', 'office_id': office_id})

    elif action == "reset":
        command_queue.put({'type': 'reset', 'office_id': office_id})

    elif action == "add_number":
        # Dodaj nowy numer do kolejki z notatką (jeśli podano)
        note = data.get('note', "")  # pusta notatka, jeśli brak
        command_queue.put({
            'type': 'add_number',
            'office_id': office_id,
            'note': note
        ...
        # Usuwanie numeru z kolejki
        if 'number' not in data:
            return jsonify({'success': False, 'error': 'Missing "number" field'}), 400
        command_queue.put({
            'type': 'remove_number',
            'office_id': office_id,
            'number': data['number']
        ...
        # Edytowanie notatki
        if 'number' not in data or 'note' not in data:
            return jsonify({'success': False, 'error': 'Missing "number" or "note" field'}), 400
        command_queue.put({
            'type': 'edit_number',
            'office_id': office_id,
            'number': data['number'],
...
class MainWindow(QtWidgets.QMainWindow):
    def __init__(self, command_queue, config):
        ...
        # Zakresy liczb dla 3 gabinetów
        self.min_values = [100, 200, 300]
        self.max_values = [199, 299, 399]

        # Początkowe wyświetlane liczby (w startowym zakresie)
        ...
        # Kolejki pacjentów
        # Każdy element: { time, number, note }
        self.office_queues = [
            [
                {'time': '08:00', 'number': 100, 'note': ''},
                {'time': '08:15', 'number': 101, 'note': ''}
            ],
            [
                {'time': '08:00', 'number': 200, 'note': ''}
            ],
            [
                {'time': '08:00', 'number': 300, 'note': ''}
            ]
        ]

        # Mapowanie ID przycisków (MQTT)
    ...
    def updateScaling(self):
        ...
        if h < 10:
            self.scale_factor = 1.0
            return
        self.scale_factor = (h / self.design_height) * self.global_scale_multiplier
    ...
    def updateFonts(self):
        ...
        self.footer_label.setStyleSheet(f"font-size: {font_footer_px}px; color: black;")
    ...
    def process_queue(self):
        while True:
            ...
            if ctype == 'update_display':
                self.update_display(command['button_id'], command['action'])

            elif ctype == 'low_battery':
                self.show_low_battery_alert(command['button_id'])

            elif ctype == 'increment':
                i = command['office_id'] - 1
                current_val = self.numbers[i]
                # Znajdź najbliższy większy numer w kolejce
                queue_vals = sorted(x['number'] for x in self.office_queues[i])
                ...
                i = command['office_id'] - 1
                current_val = self.numbers[i]
                # Znajdź najbliższy mniejszy numer w kolejce
                queue_vals = sorted(x['number'] for x in self.office_queues[i])
                ...
                used_nums = [x['number'] for x in self.office_queues[i]]
                new_num = self.min_values[i]
                # Szukaj pierwszego wolnego numeru w zakresie
                while new_num in used_nums and new_num <= self.max_values[i]:
                    new_num += 1
                if new_num <= self.max_values[i]:
                    current_time = time.strftime("%H:%M")
                    self.office_queues[i].append({
                        'time': current_time,
                        'number': new_num,
                        'note': note
                    })
                # Jeśli nie ma już wolnego, nic nie robimy (lub można dać komunikat)

            elif ctype == 'remove_number':
                i = command['office_id'] - 1
                num_to_remove = command['number']
                self.office_queues[i] = [
                    x for x in self.office_queues[i]
                    if x['number'] != num_to_remove
                ]

            elif ctype == 'edit_number':
                i = command['office_id'] - 1
                num = command['number']
                new_note = command['note']
                for item in self.office_queues[i]:
                    if item['number'] == num:
                        item['note'] = new_note
                        break

            # Odświeżamy wyświetlacze
            self.refresh_numbers()

    def refresh_numbers(self):
        # Ustaw QLabels wg self.numbers
        self.num1_label.setNumber(self.numbers[0])
        self.num2_label.setNumber(self.numbers[1])
        self.num3_label.setNumber(self.numbers[2])

    def update_display(self, button_id, action):
        # Obsługa zdarzeń z przycisków MQTT
        column_index = self.button_id_to_column.get(button_id)
        if column_index is not None:
            if action in ('single', 'double'):
                # Szukamy najbliższego większego
                current_val = self.numbers[column_index]
                queue_vals = sorted(x['number'] for x in self.office_queues[column_index])
                bigger = [n for n in queue_vals if n > current_val]
                if bigger:
                    self.numbers[column_index] = min(bigger)
            elif action == 'long':
                # Najbliższy mniejszy
                current_val = self.numbers[column_index]
                queue_vals = sorted(x['number'] for x in self.office_queues[column_index])
                smaller = [n for n in queue_vals if n < current_val]
                if smaller:
                    self.numbers[column_index] = max(smaller)
            else:
                print(f"Nieznana akcja: {action}")
                return

            # Ewentualne mignięcie
            self.flash_value(column_index)
            # Ewentualnie odtworzenie dźwięku
...
def main():
    ...
    global gui_instance
    gui = MainWindow(command_queue, config)
    gui_instance = gui
