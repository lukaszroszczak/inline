...
@app.route('/')
def index():
    # Widok główny – plik statyczny index.html (panel 3 gabinetów)
    return app.send_static_file('index.html')

@app.route('/status', methods=['GET'])
def status():
    # Zwracamy aktualne liczby i kolejki w formie JSON
    return jsonify(gui_instance.get_status())

@app.route('/office/<int:office_id>', methods=['GET'])
def office_view(office_id):
    # Widok pojedynczego gabinetu (templates/office.html)
...
# ----------------------------------------
# JEDEN ENDPOINT DO WSZYSTKICH AKCJI
# /office/<office_id>/action/<akcja>
# ----------------------------------------
...
class MainWindow(QtWidgets.QMainWindow):
    def __init__(self, command_queue, config):
        ...
        # ---- Wczytanie parametrów konfiguracyjnych ----
        ...
        # ---- Kolejki pacjentów (z pustą adnotacją) ----
        self.office_queues = [
            [{'time': '08:00', 'number': 1, 'note': ''}, {'time': '08:15', 'number': 2, 'note': ''}],
            [{'time': '08:00', 'number': 1, 'note': ''}],
            [{'time': '08:00', 'number': 1, 'note': ''}]
        ]
        ...
        # Tworzenie interfejsu Qt
        self.setup_ui()
        self.updateScaling()
        self.updateFonts()
        self.updateBackground()

        # Timery: obsługa kolejki i zegara
    ...
    def setup_ui(self):
        ...
        self.header_frame.setStyleSheet(f"QFrame#HeaderFrame {{ background-color: {self.header_bg}; }}")
    ...
    def updateFonts(self):
        ...
        self.title_label.setStyleSheet(f"color: white; font-size: {font_title_px}px; font-weight: bold;")
    ...
    ##################################
    # Obsługa kolejki (MQTT, Flask, dźwięki)
    ##################################
    def process_queue(self):
        while True:
            ...
            elif command['type'] == 'edit_number':
                # Edycja adnotacji (note)
                office_id = command['office_id'] - 1
                num_to_edit = command['number']
                new_note = command['note']
                for item in self.office_queues[office_id]:
                    if item['number'] == num_to_edit:
                        item['note'] = new_note
                        break
    ...
    def get_status(self):
        # Zwracamy stan licznika i listy pacjentów
        return {
